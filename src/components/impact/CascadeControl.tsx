...
export default function CascadeControl({
  ...
  return (
    <div
      className={`inline-flex max-w-full flex-wrap rounded-lg border border-line-soft bg-black/[0.025] p-1 ${compact ? 'text-[0.68rem]' : 'text-xs'}`}
      role="group"
    ...
    >
      {CASCADES.map((cascade) => (
        <button
          ...
          className={`whitespace-nowrap rounded-md px-2.5 py-1.5 font-mono font-semibold transition-colors ${
            mode === cascade.mode
              ? 'bg-ink text-paper'
