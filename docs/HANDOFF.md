...
- **Command palette**: real `⌘K` palette searches lessons/features/docs refs/tips, focuses search on open, traps Tab focus while open, resets cleanly on Escape, shows Done/Now/Locked states, and only exposes navigable links for unlocked lessons.
...
The Impact System is implemented and `npm run qa:rendered` now covers Plant reward, Forest update, and Team-of-20 cascade scaling in a committed rendered harness. Remaining:
1. Optional follow-up: expand the harness to assert newest-tree glow/count directly.
...
2. Optional follow-up: expand `npm run qa:rendered` to cover additional representative lesson challenges.
...
- Impact rendered QA now has a committed `npm run qa:rendered` harness for Plant reward, Forest update, and Team-of-20 cascade scaling. Remaining Impact harness work is direct newest-tree assertions.
...
2. Continue with **P4** Vercel deploy once credentials are fixed, or expand `npm run qa:rendered` with newest-tree and additional lesson-challenge assertions.
