...
npm test         # vitest run  (currently 46 passing, 12 files)
npm run lint     # eslint      (currently clean)
npm run build    # next build  (currently passes; 43 lessons prerender static)
```
...
src/
  ...
  lib/
    ...
    lessons.ts          # 43 lessons across Beginner, Feature Modules v1, and Power User Modules v1
...
tests/                  # 12 suites, 46 tests (setup.ts mocks next/font, next/link, localStorage, matchMedia)
```
...
Lessons 3, 4, and 5 plus all Power User lessons also have `challenge?: TerminalChallenge` for type-it-yourself exercises.
...
- **Lesson model + 129-tip registry + localStorage progress** (`8236f29`), now extended with track metadata, feature-family metadata, official docs references, and explicit efficiency habits.
...
- **129-tip banking pass**: inline tips are visible and bankable in each lesson, the Forest tracks 129 trees, signature tips still drive lesson completion/unlock.
...
- **Type-it-yourself terminal core**: lessons 3, 4, and 5 plus all Power User lessons now include terminal challenges with `?` hints, `reset`, incorrect feedback, and scripted success output.
...
**Verified:** 46/46 tests, lint clean, production build passes (`/`, `/how-we-calculate`, all 43 lessons prerender static). Browser QA used isolated headless Chrome DevTools Protocol because the MCP Playwright profile was locked: desktop home, mobile home, desktop/mobile lesson, reduced-motion lesson, branded 404, expanded dashboard, `agent-loop`, `checkpointing`, `custom-slash-commands`, `worktrees-isolation`, and `vs-code-integration` rendered without horizontal overflow.
...
Feature Modules v1 and Power User Modules v1 are shipped, including the first integrations slice. Next modules to add:
1. Programmatic Use / SDK: Agent SDK, headless mode, SDK sessions, SDK permissions/user input, streaming, structured output, custom tools, tool search, cost tracking, observability, hosting/storage, and secure deployment.
2. Team / Enterprise: organization setup, analytics/monitoring, managed MCP/settings, security/data usage, network gateways, and rollout kits.
...
- Impact baseline has desktop/mobile screenshot coverage for 129 trees, but the bank-tip Plant reward and cascade toggles still need rendered interaction QA.
