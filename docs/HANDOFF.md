...
npm test         # vitest run  (currently 60 passing, 13 files)
...
tests/                  # 13 suites, 60 tests (setup.ts mocks next/font, next/link, localStorage, matchMedia)
```
...
- **Dashboard feature maps**: each track summarizes feature families with module counts and bankable token-habit counts, so users can scan the Claude Code surface area before entering a lesson.
