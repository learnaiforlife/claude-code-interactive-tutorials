...
npm test         # vitest run  (currently 48 passing, 12 files)
npm run lint     # eslint      (currently clean)
npm run build    # next build  (currently passes; 55 lessons prerender static)
```
...
src/
  ...
  lib/
    ...
    lessons.ts          # 55 lessons across Beginner, Feature Modules v1, Power User Modules v1, and Team Modules v1
...
tests/                  # 12 suites, 48 tests (setup.ts mocks next/font, next/link, localStorage, matchMedia)
```
...
- **Lesson model + 165-tip registry + localStorage progress** (`8236f29`), now extended with track metadata, feature-family metadata, official docs references, and explicit efficiency habits.
...
- **165-tip banking pass**: inline tips are visible and bankable in each lesson, the Forest tracks 165 trees, signature tips still drive lesson completion/unlock.
...
- **Feature Modules v1 track**: 11 shipped modules covering Agent loop, Prompt input, Continue/resume, Slash commands, Search/read, CLAUDE.md, Context window, Built-in tools, Bash/PowerShell, Permission modes, and Checkpointing. Each teaches what it is, how it works, how to use it, and how to use it efficiently with a bankable token-saving habit.
- **Power User Modules v1 track**: 24 shipped modules covering Custom slash commands, Skills, Subagents, Hooks, MCP, Plugins, Plugin distribution, Worktrees, Agent view, Agent teams, Dynamic workflows, Goals, Scheduled tasks/routines, VS Code, JetBrains, Desktop, Chrome/computer use, GitHub Actions, GitLab CI/CD, Code Review, Slack/Remote Control, web/cloud sessions, channels, and deep links. Each includes official docs refs, a typed terminal challenge, and a token-efficiency habit.
- **Team Modules v1 track**: 12 shipped modules covering Agent SDK, headless automation, SDK sessions, SDK permissions/user input, SDK streaming, structured outputs, custom tools, tool search, cost tracking, observability, hosting/session storage, and secure deployment. Each includes official docs refs, a typed terminal challenge, and a token-efficiency habit.
...
**Verified:** 48/48 tests, lint clean, production build passes (`/`, `/how-we-calculate`, all 55 lessons prerender static). Browser QA used isolated headless Chrome DevTools Protocol because the MCP Playwright profile was locked: desktop home, mobile home, desktop/mobile lesson, reduced-motion lesson, branded 404, expanded dashboard, `agent-loop`, `checkpointing`, `custom-slash-commands`, `worktrees-isolation`, `vs-code-integration`, and `agent-sdk-overview` rendered without horizontal overflow.
...
Feature Modules v1, Power User Modules v1, and Team Modules v1 are shipped through the Programmatic Use / SDK slice. Next modules to add:
1. Team / Enterprise: organization setup, analytics/monitoring, managed MCP/settings, security/data usage, network gateways, rollout kits, GitHub Enterprise Server, Bedrock/Vertex/Foundry providers, and policy rollout.
2. Remaining feature audit: re-run the official docs index and add modules for any new or missed Claude Code pages before deploy.
...
- Impact baseline has desktop/mobile screenshot coverage for 165 trees, but the bank-tip Plant reward and cascade toggles still need rendered interaction QA.
